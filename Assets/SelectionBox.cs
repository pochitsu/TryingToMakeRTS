...
public class SelectionBox : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (Input.GetMouseButton(0))
        {
            ...
            {     
                //SelectionMovementManager.Instance.DeSelectAll();
                SelectUnits();
            }
    ...
    void SelectUnits()
    {
        foreach (var unit in SelectionMovementManager.Instance.allUnitList)
        {
            if (selectionBox.Contains(myCam.WorldToScreenPoint(unit.transform.position)))
            {
                SelectionMovementManager.Instance.DragSelect(unit);
            }
        }
    }
}